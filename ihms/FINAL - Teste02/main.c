...
#include <stdlib.h>
#include <stdio.h>
#include "flanger.h" // Adicionar a inclusão do cabeçalho do flanger
...
void main()
{
    ...
    float flangerDelayMs = 5.0f;    // Parâmetros do flanger
    float flangerDepthMs = 2.0f;
    ...
    printf("Exp --- experimento de efeito flanger\n");
    ...
    while ((fread(&temp, sizeof(Int8), 2*MAX_BUF_SIZE, fpIn)) == 2*MAX_BUF_SIZE) {
        for (j = 0, i = 0; i < MAX_BUF_SIZE; i++) {
            ...
            // Aplica o efeito flanger à amostra
            apply_flanger(&sample, 1, sampleRate, flangerDelayMs, flangerDepthMs, flangerRateHz);
